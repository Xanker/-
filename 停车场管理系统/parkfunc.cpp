...
void Manager::insertsort()
{
	...
	{
		car* tem = p->parkinglot[i];
		int key = p->parkinglot[i]->getSpace();
		int j = i - 1;
		while (j >= 0 && p->parkinglot[j]->getSpace() < key)
		{
...
void Manager::quicksort(int low, int high)
{
	if (low >= high)return;
	int i = low, j = high;
	car* tem = p->parkinglot[i];
	...
	while (i < j)
	{	
		while (i<j && p->parkinglot[j]->getSpace()>=key)
		{
			j--;
		...
		while(i<j && p->parkinglot[i]->getSpace()<=key)
		{
			i++;
		}
              p->parkinglot[j] = p->parkinglot[i];
	}
	p->parkinglot[i] = tem;
	quicksort(low, i - 1);
	quicksort(i + 1, high);
