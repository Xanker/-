...
void Manager::savecarlog(int i)
{
	...
	if (file.is_open()) {
		...
		file << p->parkinglot[i - 1]->getNumber() << ","
			<< p->parkinglot[i - 1]->gethour() << ":"
			<< p->parkinglot[i - 1]->getmin()
			<< "," << p->parkinglot[i - 1]->getdeparthour() << ":"
			<< p->parkinglot[i - 1]->getdepartmin() << ","
			<< p->parkinglot[i - 1]->getAmount() << "\n";
